...
//DEFER STATEMENT - delays execution until surrounding function returns
func listName(name string) {
	fmt.Println(name)
}
func listAge(age int) {
	fmt.Println(age)
}
func listHobby(hobby string) {
	fmt.Println(hobby)
}
...
//Anonymous functions - without any identifiersto refer to it - short term use- un-named functionality
	...
	listName("Jason")
	defer listAge(20)   //to delay until subsequent functions executed
	listHobby("Fishing")
